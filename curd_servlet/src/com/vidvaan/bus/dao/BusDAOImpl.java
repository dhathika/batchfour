	...
	public List<Bus> findAll() {
		...
		try {
			...
			while (resultSet.next()) {
				Bus bus = new Bus();
				bus.setBid(resultSet.getInt("bus_id"));
				bus.setBname(resultSet.getString("bus_name"));
				bus.setBtype(resultSet.getString("bus_type"));
				bus.setRoute(resultSet.getString("bus_route"));
				//java.util.Date todate = resultSet.getDate(5);
				bus.setTodate(resultSet.getString("bus_todate"));
				bus.setFromdate(resultSet.getString("bus_fromdate"));
...
//				bus.setFromdate(DateConversionMethods.stringDate(fromdate));
				bus.setBcapacity(resultSet.getInt("bus_capacity"));
				System.out.println(" Read one record success");
				list.add(bus);
